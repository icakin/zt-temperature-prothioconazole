...
# The composites are still assembled from the (unsaved) intermediate plot objects.
# To restore a figure, add its file name here (or remove this override block).
.KEEP_FIGS <- c(
  "fig1_composite.png",
  "fig3_bliss_composite.png",
  "combined_composite.png"
)
...
# --- Paths -------------------------------------------------------------------
# Input: fig1_extra_dose_response_by_temp.csv produced by 07_plots_and_effects.R
data_path <- file.path(tables_dir, "fig1_extra_dose_response_by_temp.csv")

# Growth rate outputs (subdirectories under figures/tables/models)
fig_dir   <- file.path(figures_dir, "growth")
mod_dir   <- file.path(models_dir, "growth")
tab_dir   <- file.path(tables_dir, "growth")

# Respiration rate outputs
fig_dir_resp <- file.path(figures_dir, "respiration")
tab_dir_resp <- file.path(tables_dir, "respiration")
mod_dir_resp <- file.path(models_dir, "respiration")
...
# Panel: pEC50 by temperature + global mean as an offset point on the right.
# Numeric temperature axis (so the linear trend renders perfectly straight); the
# pooled Global estimate is placed just past 28 C, beyond a dashed separator.
.x_global <- 30.5
pec50_plot_data <- pec50_report %>%
  transmute(temp_lab = as.character(temperature),
            xpos     = as.numeric(as.character(temperature)),
            pEC50_mean, pEC50_sd) %>%
  bind_rows(tibble(temp_lab = "Global", xpos = .x_global,
                   pEC50_mean = global_pec50_mean, pEC50_sd = global_pec50_sd)) %>%
  mutate(temp_lab = factor(temp_lab,
                           levels = c("15", "18", "21", "24", "26", "27", "28", "Global")))

# Overall trend of sensitivity over temperature: linear lm(pEC50 ~ temperature)
# fitted on EACH posterior draw (per-draw approach as in 00_config.R), drawn as a
# single straight segment over 15-28 C from the posterior-median intercept/slope.
pec50_trend_draws <- pec50_draws %>%
  mutate(temp_num = as.numeric(as.character(temperature))) %>%
  group_by(.draw) %>%
  group_modify(~{
    m <- lm(pec50 ~ temp_num, data = .x)
    tibble(intercept = coef(m)[[1]], slope = coef(m)[[2]])
  }) %>%
  ungroup()

pec50_trend_int   <- median(pec50_trend_draws$intercept)
pec50_trend_slope <- median(pec50_trend_draws$slope)
pec50_trend_pdec  <- mean(pec50_trend_draws$slope < 0)

pb_growth <- ggplot(pec50_plot_data, aes(x = xpos, y = pEC50_mean,
                                         colour = temp_lab)) +
  geom_vline(xintercept = 29.25, linetype = "dashed", colour = "grey70") +
  annotate("segment", x = 15, xend = 28,
           y    = pec50_trend_int + pec50_trend_slope * 15,
           yend = pec50_trend_int + pec50_trend_slope * 28,
  ...
  geom_errorbar(aes(ymin = pEC50_mean - 2 * pEC50_sd,
                ...
                width = 0.25, linewidth = 0.4, linetype = "dashed") +
  scale_colour_manual(
    values = c(temp_pal, "Global" = "black"),
    guide = "none"
  ) +
  scale_x_continuous(breaks = c(15, 18, 21, 24, 26, 27, 28, 30.5),
                     labels = c("15", "18", "21", "24", "26", "27", "28", "Global")) +
  annotate("text", x = Inf, y = Inf, label = "More sensitive \u2191",
...
# Panel: Slope by temperature + global mean as an offset point on the right.
# Numeric temperature axis so the linear trend renders perfectly straight.
slope_report_plot <- slope_report %>%
  transmute(temp_lab = as.character(temperature),
            xpos     = as.numeric(as.character(temperature)),
            slope_mean, slope_sd) %>%
  bind_rows(tibble(temp_lab = "Global", xpos = .x_global,
                   slope_mean = global_slope_mean, slope_sd = global_slope_sd)) %>%
  mutate(temp_lab = factor(temp_lab,
                           levels = c("15", "18", "21", "24", "26", "27", "28", "Global")))

# Overall trend of the dose slope over temperature: per-draw lm(slope ~ temp),
# drawn as a single straight segment over 15-28 C from the median intercept/slope.
slope_trend_draws <- slope_draws %>%
  mutate(temp_num = as.numeric(as.character(temperature))) %>%
  group_by(.draw) %>%
  group_modify(~{
    m <- lm(slope ~ temp_num, data = .x)
    tibble(intercept = coef(m)[[1]], trend = coef(m)[[2]])
  }) %>%
  ungroup()

slope_trend_int  <- median(slope_trend_draws$intercept)
slope_trend_med  <- median(slope_trend_draws$trend)
slope_trend_pdec <- mean(slope_trend_draws$trend < 0)

pb_resp <- ggplot(slope_report_plot, aes(x = xpos, y = slope_mean,
                                         colour = temp_lab)) +
  geom_vline(xintercept = 29.25, linetype = "dashed", colour = "grey70") +
...
# =============================================================================
# PUBLICATION FIGURE 1 — TPC + Dose-Response Composite
# =============================================================================
# Combines: (A-D) Sharpe-Schoolfield TPCs with lnB0 posteriors from script 05
#           (E-H) Growth & Respiration dose-response + parameter estimates
#
# Layout:
#   Row 1: Growth TPC (A)         | lnB0 growth posterior (B)
#   Row 2: Respiration TPC (C)    | lnB0 respiration posterior (D)
#   Row 3: Growth dose-response (E) | pEC50 forest plot (F)
#   Row 4: Resp. dose-response (G)  | Slope estimates (H)
# =============================================================================
...
# Recover TPC plot objects (available in environment from run_all.R,
# or load from saved RDS for standalone execution)
if (!exists("p_tpc_g_pub")) {
  rds_path <- file.path(models_dir, "plot_tpc_growth_pub.rds")
  if (file.exists(rds_path)) p_tpc_g_pub <- readRDS(rds_path)
}
if (!exists("p_tpc_r_pub")) {
  rds_path <- file.path(models_dir, "plot_tpc_resp_pub.rds")
  if (file.exists(rds_path)) p_tpc_r_pub <- readRDS(rds_path)
}
if (!exists("p_lnB0_g")) {
  rds_path <- file.path(models_dir, "plot_lnB0_growth.rds")
  if (file.exists(rds_path)) p_lnB0_g <- readRDS(rds_path)
}
if (!exists("p_lnB0_r")) {
  rds_path <- file.path(models_dir, "plot_lnB0_resp.rds")
  if (file.exists(rds_path)) p_lnB0_r <- readRDS(rds_path)
}

can_build_fig1 <- all(sapply(
  c("p_tpc_g_pub", "p_tpc_r_pub", "p_lnB0_g", "p_lnB0_r",
    "pa_growth", "pb_growth", "pa_resp", "pb_resp"),
  exists
))

if (can_build_fig1) {

  # --- Top half: Sharpe-Schoolfield TPCs with lnB0 posteriors ----------------
  # Keep ONE dose legend on panel A (top) so the A-D colours are decodable;
  # panel C shares it, so its legend is stripped.
  tpc_g <- p_tpc_g_pub +
    ...
    guides(fill = "none",
           colour = guide_legend(nrow = 1, override.aes = list(size = 2)))
  tpc_r <- p_tpc_r_pub + theme(legend.position = "none")

  # Use patchwork for the TPC rows (matching the 2:1 width ratio)
  row_tpc_g <- tpc_g + p_lnB0_g + plot_layout(widths = c(2, 1))
  row_tpc_r <- tpc_r + p_lnB0_r + plot_layout(widths = c(2, 1))

  # --- Dose-response rows: growth/respiration dose-response + parameters ------
  # Add the shared Temperature legend to the growth dose-response panel (top).
  pa_growth_leg <- pa_growth +
    theme(
      legend.position = "top",
      legend.title    = element_text(size = 9),
  ...
  row_dr_g <- pa_growth_leg + pb_growth + plot_layout(widths = c(2, 1))
  row_dr_r <- pa_resp       + pb_resp   + plot_layout(widths = c(2, 1))

  pub_fig_dir <- file.path(base_dir, "figures")

  # --- ONE unified Figure 1: four stacked rows, panels A-H -------------------
  #   Row 1 (A,B): growth TPC              | lnB0 growth
  #   Row 2 (C,D): respiration TPC         | lnB0 respiration
  #   Row 3 (E,F): growth dose-response    | pEC50 by temperature
  #   Row 4 (G,H): respiration dose-resp.  | dose slope by temperature
  # Figure 1 and the dose-response composite are now a SINGLE figure (A-H),
  # not two separate images patched together.
  fig1_full <- cowplot::plot_grid(
    patchwork::patchworkGrob(row_tpc_g),
    patchwork::patchworkGrob(row_tpc_r),
    patchwork::patchworkGrob(row_dr_g),
    patchwork::patchworkGrob(row_dr_r),
    ncol = 1,
    labels = c("A", "C", "E", "G"),
    label_size = 16, label_fontface = "bold",
    label_x = 0, label_y = 1
  ) +
    cowplot::draw_label("B", x = 0.64, y = 1.000, fontface = "bold", size = 16) +
    cowplot::draw_label("D", x = 0.64, y = 0.750, fontface = "bold", size = 16) +
    cowplot::draw_label("F", x = 0.64, y = 0.500, fontface = "bold", size = 16) +
    cowplot::draw_label("H", x = 0.64, y = 0.250, fontface = "bold", size = 16)

  ggsave(
    file.path(pub_fig_dir, "fig1_composite.png"),
    fig1_full, width = 14, height = 18, dpi = 300, bg = "white"
  )
  cat("\n--- Figure 1 (A-H: TPCs + lnB0 + dose-response + parameters) saved ---\n")

} else {
  warning("Could not build composite Fig. 1: TPC plot objects not available.\n",
          "Run 07_plots_and_effects.R first or use run_all.R.")
}

# =============================================================================
# PUBLICATION FIGURE 3 — Bliss Independence: Deviation + Heatmap
# =============================================================================
# Combines: (A) Bliss deviation vs temperature (p19)
#           (B) Mean Bliss delta heatmap (p21)
#
# Layout:  Panel A (wider, line plot) | Panel B (heatmap, equal aspect)
# =============================================================================

if (exists("p19") && exists("p21")) {

  # Publication styling: remove titles/subtitles, use panel labels instead
  p19_pub <- p19 +
  ...
  pub_fig_dir <- file.path(base_dir, "figures")
  ggsave(
    file.path(pub_fig_dir, "fig3_bliss_composite.png"),
    fig3_pub, width = 14, height = 6, dpi = 300, bg = "white"
  )
  cat("\n--- Publication Figure 3 (Bliss composite) saved ---\n")

} else {
  warning("Could not build composite Fig. 3: Bliss plot objects (p19, p21) not available.")
}

# =============================================================================
# Sync final publication figures into ../manuscript/figures so the manuscript
# copies never drift out of date. Only figures with a KNOWN mapping are copied;
# add rows to fig_sync_map to include Figure4/Figure5 once their sources are set.
# =============================================================================
manuscript_fig_dir <- file.path(dirname(base_dir), "manuscript", "figures")
fig_sync_map <- c(
  "fig1_composite.png"               = "Figure1_thermal_performance.png",
  "fig2_cue_alpha.png"               = "Figure2_CUE.png",
  "fig3_bliss_composite.png"         = "Figure3_bliss_interaction.png"
)
if (dir.exists(manuscript_fig_dir)) {
  message("\nSyncing publication figures -> ", manuscript_fig_dir)
  for (src_name in names(fig_sync_map)) {
    src <- file.path(base_dir, "figures", src_name)
    dst <- file.path(manuscript_fig_dir, fig_sync_map[[src_name]])
    if (file.exists(src)) {
      ok <- file.copy(src, dst, overwrite = TRUE)
      message(if (ok) "  synced  " else "  FAILED  ", src_name, " -> ", basename(dst))
    } else {
      message("  skipped (not generated): ", src_name)
    }
  }
} else {
  message("Manuscript figures folder not found; skipped sync: ", manuscript_fig_dir)
}
