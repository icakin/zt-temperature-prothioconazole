...
# ── MANUAL TRIMMING ──────────────────────────────────────────────────────
# 02 is AUTOMATIC ONLY. Do all manual fit-window trimming interactively in
# 03_trim_selector.R, which writes tables/manual_fit_windows.csv; 03 applies
# those windows. This table is intentionally empty (no hardcoded overrides).
...
# Filtered output carries RAW oxygen only (no O2_fit spline column). The spline
# is used internally above solely to auto-detect the trim window.
filtered <- trimmed %>%
...
for (sid in names(diag_lst)) {
  ...
  p <- ggplot(df, aes(Time, Oxygen)) +
    ...
    geom_line(colour = "grey30", linewidth = 0.8)   # RAW oxygen trace only
