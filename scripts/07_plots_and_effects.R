...
# ---- Figure whitelist ------------------------------------------------------
# Only the figures below are written; every other ggsave() call silently no-ops.
# All computation/plot objects still run, so the composites can still be built.
# To restore a figure, add its file name here (or remove this override block).
.KEEP_FIGS <- c(
  "bayesian_sharpe_schoolfield_tpc_growth_C_per_C_h_by_dose.png",
  "bayesian_sharpe_schoolfield_tpc_respiration_C_per_C_h_by_dose.png",
  "posterior_distributions_sharpe_schoolfield_growth_C_per_C_h.png",
  "posterior_distributions_sharpe_schoolfield_respiration_C_per_C_h.png",
  "bayesian_arrhenius_CUE_common_slope_vs_temperature.png",
  "fig1_tpc_lnB0.png",
  "fig2_cue_alpha.png",
  # supplementary fit-diagnostics figures
  "supp_tpc_fit_growth.png",
  "supp_tpc_fit_respiration.png"
)
ggsave <- function(filename, ...) {
  if (basename(filename) %in% .KEEP_FIGS) ggplot2::ggsave(filename, ...)
  else invisible(NULL)
}

# Lift R's vector-memory cap on macOS before loading the large stage-2 workspace
# (otherwise: "vector memory limit of 16.0 Gb reached").
if (Sys.info()[["sysname"]] == "Darwin") {
  try(suppressWarnings(mem.maxVSize(vsize = Inf)), silent = TRUE)
}

# Reload the Stage 2 workspace: prediction grids, datasets, dose metadata,
# fitted brms models, and the TPC-check plotting helper. 06 saves a curated
# object list (an explicit save(), not save.image()), so this file is small and
# loads comfortably within the vector-memory cap.
...
# ===== Biomass-corrected boxplots =============================================
# Built here (rather than inherited from 04_oxygen_fits.R) so this script runs
# standalone. Depends only on results_plot, dose_cols, and dose_levels, all of
# which are restored from stage2_workspace.RData above.
...
# ── Extra (non-kept) effect-size / synergy / rate analyses — gated to speed 05 ─
# These feed only removed figures and read posterior CSVs from the skipped models
# (so they'd also error if those CSVs are absent). The whole block is self-contained
# (verified nothing after it uses anything defined here).
if (isTRUE(FIT_ALL_MODELS)) {
...
}  # end FIT_ALL_MODELS gate part 1 (region synergy)

# CUE common-slope figure (KEPT) — built earlier; saved OUTSIDE the gate so it
# is always produced regardless of FIT_ALL_MODELS.
...
if (isTRUE(FIT_ALL_MODELS)) {  # gate part 2: remaining non-kept analyses
...
}  # end FIT_ALL_MODELS gate (extra effect-size / synergy / rate analyses)
...
make_alpha_halfeye <- function(post_csv) {
  ...
  ggplot(post_df, aes(x = alpha, y = Dose, fill = Dose, colour = Dose)) +
    ...
    scale_fill_manual(values = dose_cols, limits = dose_levels, drop = FALSE) +
    scale_colour_manual(values = dose_cols, limits = dose_levels, drop = FALSE) +
    labs(x = expression(alpha ~ "≡" ~ ln(B[0])), y = NULL) +
    theme_classic(11) +
    theme(legend.position = "none", axis.text.y = element_text(size = 9))
}
...
if (!is.null(p_lnB0_g) && !is.null(p_lnB0_r)) {
  ...
  ggsave(
    ...
    fig1_tpc, width = 13, height = 9, dpi = 300, bg = "white"
  )
  message("Saved: ", file.path(pub_fig_dir, "fig1_tpc_lnB0.png"))

  # Save individual TPC panel objects for composite assembly in 06
  saveRDS(p_tpc_g_pub, file.path(models_dir, "plot_tpc_growth_pub.rds"))
  saveRDS(p_tpc_r_pub, file.path(models_dir, "plot_tpc_resp_pub.rds"))
  saveRDS(p_lnB0_g,    file.path(models_dir, "plot_lnB0_growth.rds"))
  saveRDS(p_lnB0_r,    file.path(models_dir, "plot_lnB0_resp.rds"))
  message("Saved TPC plot objects for composite Fig. 1 assembly")
}
...
# ---------------------------------------------------------------------------
# Per-treatment TPC thermal traits from the Sharpe-Schoolfield fits:
#   (a) activation energy E (eV) and (b) thermal optimum Topt (°C),
# one value per dose, for growth and respiration. Points = posterior mean,
# bars = 95% credible interval.
# ---------------------------------------------------------------------------
read_ss_param <- function(path, prefix, to_celsius = FALSE) {
  if (!file.exists(path)) return(NULL)
  s <- readr::read_csv(path, show_col_types = FALSE)
  if (!"variable" %in% names(s)) return(NULL)
  out <- s %>%
    dplyr::filter(grepl(paste0("^", prefix), variable)) %>%
    dplyr::mutate(Dose_key = sub(prefix, "", variable),
                  val = mean, lo = q2.5, hi = q97.5)
  if (to_celsius) out <- out %>%
    dplyr::mutate(val = ifelse(val > 200, val - 273.15, val),
                  lo  = ifelse(lo  > 200, lo  - 273.15, lo),
                  hi  = ifelse(hi  > 200, hi  - 273.15, hi))
  out %>% dplyr::left_join(dose_key_tbl, by = "Dose_key") %>%
    dplyr::transmute(Dose = factor(Dose, levels = dose_levels), val, lo, hi)
}
# TRUE thermal optimum (curve PEAK), computed per posterior draw from the
# Schoolfield parameters. Th is the deactivation reference (above the peak); the
# peak is Topt = Eh*Th / (Eh + kB*Th*ln(Eh/E - 1)). Uses the posterior draws so
# the credible interval is correct.
read_Topt_peak <- function(path) {
  if (!file.exists(path)) return(NULL)
  d <- readr::read_csv(path, show_col_types = FALSE)
  if (!all(c("Dose", "E", "Eh", "Th") %in% names(d))) return(NULL)
  kB <- 8.617e-5
  d %>%
    dplyr::filter(is.finite(E), is.finite(Eh), is.finite(Th), Eh > E) %>%
    dplyr::mutate(Topt = (Eh * Th) / (Eh + kB * Th * log(Eh / E - 1)) - 273.15) %>%
    dplyr::filter(is.finite(Topt), Topt > 10, Topt < 45) %>%
    dplyr::group_by(Dose) %>%
    dplyr::summarise(val = stats::median(Topt),
                     lo  = stats::quantile(Topt, 0.025),
                     hi  = stats::quantile(Topt, 0.975), .groups = "drop") %>%
    dplyr::mutate(Dose = factor(Dose, levels = dose_levels))
}
.ss_g <- file.path(tables_dir, "summary_sharpe_schoolfield_tpc_growth_C_per_C_h_by_dose.csv")
.ss_r <- file.path(tables_dir, "summary_sharpe_schoolfield_tpc_respiration_C_per_C_h_by_dose.csv")
E_g <- read_ss_param(.ss_g, "b_E_Dose_key");  E_r <- read_ss_param(.ss_r, "b_E_Dose_key")
T_g <- read_Topt_peak(file.path(tables_dir, "posterior_sharpe_schoolfield_tpc_growth_C_per_C_h_by_dose.csv"))
T_r <- read_Topt_peak(file.path(tables_dir, "posterior_sharpe_schoolfield_tpc_respiration_C_per_C_h_by_dose.csv"))
if (!is.null(E_g) && !is.null(E_r) && !is.null(T_g) && !is.null(T_r)) {
  dose_pal <- c(Growth = "#009E73", Respiration = "#D55E00")
  pd <- position_dodge(width = 0.5)
  base_theme <- theme_classic(12) +
    theme(axis.line = element_line(linewidth = 0.4),
          axis.ticks = element_line(linewidth = 0.4),
          axis.text  = element_text(colour = "black"),
          plot.title = element_text(face = "bold", size = 12))
  E_df <- dplyr::bind_rows(dplyr::mutate(E_g, Rate = "Growth"),
                           dplyr::mutate(E_r, Rate = "Respiration"))
  T_df <- dplyr::bind_rows(dplyr::mutate(T_g, Rate = "Growth"),
                           dplyr::mutate(T_r, Rate = "Respiration"))
  .lab_dose <- factor(tail(dose_levels, 1), levels = dose_levels)
  pA <- ggplot(E_df, aes(Dose, val, colour = Rate, group = Rate)) +
    geom_hline(yintercept = 0.65, linetype = "22", linewidth = 0.3, colour = "grey75") +
    geom_text(data = data.frame(Dose = .lab_dose, val = 0.66),
              aes(Dose, val), label = "~0.65 eV", inherit.aes = FALSE,
              size = 2.7, colour = "grey55", hjust = 1, vjust = 0) +
    geom_errorbar(aes(ymin = lo, ymax = hi), width = 0.16, linewidth = 0.5, position = pd) +
    geom_point(size = 2.3, position = pd) +
    scale_colour_manual(values = dose_pal) +
    labs(x = expression("Prothioconazole (mg L"^{-1}*")"),
         y = expression("Activation energy, "*italic(E)*" (eV)"), colour = NULL, title = "a") +
    base_theme +
    theme(legend.position = c(0.02, 0.98), legend.justification = c(0, 1),
          legend.background = element_blank(), legend.key = element_blank())
  pB <- ggplot(T_df, aes(Dose, val, colour = Rate, group = Rate)) +
    geom_hline(yintercept = 28, linetype = "dotted", linewidth = 0.4, colour = "grey70") +
    geom_text(data = data.frame(Dose = .lab_dose, val = 28.05),
              aes(Dose, val), label = "max measured 28 °C", inherit.aes = FALSE,
              size = 2.6, colour = "grey55", hjust = 1, vjust = 0) +
    geom_errorbar(aes(ymin = lo, ymax = hi), width = 0.16, linewidth = 0.5, position = pd) +
    geom_point(size = 2.3, position = pd) +
    scale_colour_manual(values = dose_pal, guide = "none") +
    labs(x = expression("Prothioconazole (mg L"^{-1}*")"),
         y = expression("Thermal optimum, "*italic(T)[opt]*" (°C)"), title = "b") +
    base_theme
  p_ET <- pA + pB + patchwork::plot_layout(widths = c(1, 1))
  ggsave(file.path(figures_dir, "tpc_E_Topt_by_dose.png"),
         p_ET, width = 11, height = 4.7, dpi = 600, bg = "white")
  message("Saved: ", file.path(figures_dir, "tpc_E_Topt_by_dose.png"))

  # Tidy per-dose thermal-traits table (growth & respiration), each trait with a
  # 95% credible interval: activation energy E, deactivation Eh, TRUE thermal
  # optimum Topt (curve peak), and baseline log-rate lnB0.
  lnB0_df <- dplyr::bind_rows(
    dplyr::mutate(read_ss_param(.ss_g, "b_lnB0_Dose_key"), Rate = "Growth"),
    dplyr::mutate(read_ss_param(.ss_r, "b_lnB0_Dose_key"), Rate = "Respiration"))
  Eh_df <- dplyr::bind_rows(
    dplyr::mutate(read_ss_param(.ss_g, "b_Eh_Dose_key"), Rate = "Growth"),
    dplyr::mutate(read_ss_param(.ss_r, "b_Eh_Dose_key"), Rate = "Respiration"))
  traits_tbl <- E_df %>% dplyr::transmute(Rate, Dose, E = val, E_lo = lo, E_hi = hi) %>%
    dplyr::full_join(Eh_df %>% dplyr::transmute(Rate, Dose, Eh = val, Eh_lo = lo, Eh_hi = hi),
                     by = c("Rate", "Dose")) %>%
    dplyr::full_join(T_df %>% dplyr::transmute(Rate, Dose, Topt_C = val, Topt_lo = lo, Topt_hi = hi),
                     by = c("Rate", "Dose")) %>%
    dplyr::full_join(lnB0_df %>% dplyr::transmute(Rate, Dose, lnB0 = val, lnB0_lo = lo, lnB0_hi = hi),
                     by = c("Rate", "Dose")) %>%
    dplyr::arrange(Rate, Dose)
  readr::write_csv(traits_tbl, file.path(tables_dir, "tpc_thermal_traits_by_dose.csv"))
  message("Saved: ", file.path(tables_dir, "tpc_thermal_traits_by_dose.csv"))
} else {
  message("TPC E/Topt-by-dose plot skipped (SS summary files not found).")
}

# ---------------------------------------------------------------------------
# SUPPLEMENTARY: Sharpe-Schoolfield fit diagnostics — observed vs fitted curve
# per dose, and a per-dose R2/RMSE table (both on the log scale the model fits).
# ---------------------------------------------------------------------------
.ss_predict <- function(lnB0, E, Eh, Th, TK) {
  lnB0 - E * 11604.51812 * (1 / TK - 1 / 293.15) -
    log(1 + exp(Eh * 11604.51812 * (1 / Th - 1 / TK)))
}
.ss_params <- function(summ_path) {
  if (!file.exists(summ_path)) return(NULL)
  s <- readr::read_csv(summ_path, show_col_types = FALSE)
  if (!"variable" %in% names(s)) return(NULL)
  gp <- function(pref) s %>% dplyr::filter(grepl(paste0("^", pref), variable)) %>%
    dplyr::transmute(Dose_key = sub(pref, "", variable), v = mean)
  gp("b_lnB0_Dose_key") %>% dplyr::rename(lnB0 = v) %>%
    dplyr::left_join(gp("b_E_Dose_key")  %>% dplyr::rename(E  = v), by = "Dose_key") %>%
    dplyr::left_join(gp("b_Eh_Dose_key") %>% dplyr::rename(Eh = v), by = "Dose_key") %>%
    dplyr::left_join(gp("b_Th_Dose_key") %>% dplyr::rename(Th = v), by = "Dose_key") %>%
    dplyr::left_join(dose_key_tbl, by = "Dose_key") %>%
    dplyr::mutate(Dose = factor(Dose, levels = dose_levels))
}
make_ss_fit_diag <- function(dat, summ_path, ratelab, tag) {
  pars <- .ss_params(summ_path)
  if (is.null(dat) || is.null(pars) || nrow(dat) == 0) return(NULL)
  d <- dat %>% dplyr::mutate(Dose = factor(as.character(Dose), levels = dose_levels))
  Tg <- seq(min(d$T, na.rm = TRUE), max(d$T, na.rm = TRUE), length.out = 120)
  grid <- pars %>% tidyr::crossing(T = Tg) %>%
    dplyr::mutate(rate = exp(.ss_predict(lnB0, E, Eh, Th, T + 273.15)))
  p <- ggplot(d, aes(T, y_raw)) +
    geom_point(size = 1.2, alpha = 0.7, colour = "grey30") +
    geom_line(data = grid, aes(T, rate), colour = "#D55E00", linewidth = 0.8) +
    facet_wrap(~ Dose, scales = "free_y") +
    scale_y_log10() +
    labs(x = "Temperature (°C)", y = paste0(ratelab, " (log scale)"),
         title = paste0("Sharpe–Schoolfield fit vs data — ", tag),
         subtitle = "Points: observed replicates; line: posterior-mean fit") +
    theme_bw(11)
  ggsave(file.path(figures_dir, paste0("supp_tpc_fit_", tag, ".png")),
         p, width = 9.5, height = 6.5, dpi = 300, bg = "white")
  d %>% dplyr::left_join(pars, by = "Dose") %>%
    dplyr::mutate(pred = .ss_predict(lnB0, E, Eh, Th, T + 273.15)) %>%
    dplyr::group_by(Rate = tag, Dose) %>%
    dplyr::summarise(n = dplyr::n(),
                     R2 = 1 - sum((y - pred)^2) / sum((y - mean(y))^2),
                     RMSE_log = sqrt(mean((y - pred)^2)), .groups = "drop")
}
.fq_g <- make_ss_fit_diag(growth_biomass_dat, .ss_g, "Growth (C C⁻¹ h⁻¹)", "growth")
.fq_r <- make_ss_fit_diag(resp_biomass_dat,   .ss_r, "Respiration (C C⁻¹ h⁻¹)", "respiration")
if (!is.null(.fq_g) || !is.null(.fq_r)) {
  readr::write_csv(dplyr::bind_rows(.fq_g, .fq_r),
                   file.path(tables_dir, "supp_tpc_fit_quality.csv"))
  message("Saved: ", file.path(tables_dir, "supp_tpc_fit_quality.csv"))
}

# ---------------------------------------------------------------------------
# Fig. 4: Effect size vs temperature with GAM smooth
# (was Fig. 3; renumbered after composite Bliss figure inserted as Fig. 3)
# ---------------------------------------------------------------------------
...
if (isTRUE(FIT_ALL_MODELS) && !is.null(pub_eff_df)) {
  ...
  ggsave(
    file.path(pub_fig_dir, "fig4_effect_size_gam.png"),
    fig3a, width = 13, height = 5, dpi = 300, bg = "white"
  )
  message("Saved: ", file.path(pub_fig_dir, "fig4_effect_size_gam.png"))
}
...
# Write the dose-response CSV needed by 08_dose_response_brms.R
...
# The CUE-vs-dose figure (fig_cue_vs_dose) is NOT in the kept set and fits its
# own MCMC model on every run. Skip the whole block unless FIT_ALL_MODELS is on
# (this is the main reason 05 was slow).
if (isTRUE(FIT_ALL_MODELS)) {
...
}  # end FIT_ALL_MODELS gate (CUE-vs-dose extra figure + its MCMC fit)


message("07_plots_and_effects.R complete.")
