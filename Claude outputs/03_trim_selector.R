...
# =============================================================================
# STABLE-r WINDOW OPTIMISER  (curve-intrinsic)   [ported from Candidas 04]
# =============================================================================
# "Auto" above is a GEOMETRY rule: where does the curve bend and level off? It
# never asks whether the r it produces is trustworthy, and a clean fit to the
# WRONG window gives a precise but wrong r.
#
# This asks instead: which window gives an r I can BELIEVE?
#   - try many candidate windows (several starts x several ends),
#   - fit the exponential model in each, keep the ones that fit,
#   - pick the window whose r sits at the CENTRE of the good windows.
# If r barely moves as the window is nudged, that r is real (stability_cv small).
# If r swings wildly, this curve's r is not a measurement, and it says so.
#
# --- ADAPTED FOR THIS PROJECT -------------------------------------------------
# Candidas used start offsets of 0-12 MINUTES after the O2 peak, because there
# the peak is sharp and the exponential acceleration begins immediately.
# Here the inoculum is much thinner (OD 0.0005) and the trace has a long, nearly
# flat lag, so the "peak" is poorly located: measured on the round-1 15C plate
# the detected peak landed at 3, 79, 182 and 338 min on four wells of the same
# plate, and the windows actually used ran 230-490 min. Opening at peak+12 would
# therefore sit in the flat lag, not the exponential phase.
# So: wider peak-anchored offsets PLUS drawdown-anchored starts, which do not
# depend on locating the peak at all.
# Starts are DRAWDOWN-ANCHORED only (no peak+offset candidates). Measured on
# these plates: including peak+0 opens the window inside the lag phase (the 15 C
# peak is at t = 0), r then depends strongly on where you start, and the median
# stability CV was 0.30 with only 4/42 curves stable. Drawdown-anchored starts
# adapt to each curve's own lag: median CV 0.10, 21/42 stable, median rt 1.70.
STAB_START_OFFSETS_MIN <- numeric(0)                     # disabled (see above)
STAB_START_DD_FRACS    <- c(0.05, 0.10, 0.15, 0.20)      # start once O2 has
                                                         # fallen this fraction
STAB_END_FRACS         <- c(0.70, 0.80, 0.90, 0.95)      # of total drawdown;
                                                         # round-1 ends were at
                                                         # ~95% drawdown
STAB_R2_MIN            <- 0.90   # looser than Auto's 0.99 on purpose: the 27 C
                                 # plate carries an incubator temperature-cycling
                                 # sawtooth that inflates residuals without
                                 # affecting the underlying r.
STAB_MIN_PTS           <- 50

# rt = r * window = the number of E-FOLDINGS of growth the fit spans.
#   rt ~ 2   -> biomass grows ~7-fold across the window (healthy exponential)
#   rt ~ 4.5 -> biomass grows ~90-fold (the model is extrapolating hard)
# The model assumes UNLIMITED exponential growth; past ~3 e-foldings that is
# untenable and K (= consumption rate at the window START, inferred by
# extrapolating BACKWARDS) comes out noisy and biased.
# Round-1 windows here sat at rt ~ 2.2 (r ~ 0.00072/min x ~3100 min), i.e.
# already in the range this cap protects, so it should bite rarely.
STAB_RT_TARGETS <- c(3.0)
STAB_MAX_RT     <- 3.0

# ZERO-GROWTH thresholds.
#   CURV_MIN_RT     : below this, r*window cannot be told from a straight line
#                     (constant respiration, no growth). R2/RMSE cannot see this,
#                     a straight line fits the model beautifully.
#   MIN_O2_DRAWDOWN : below this (mg/L consumed from the peak onward) there is no
#                     metabolic signal and any r is fitted noise. Typical curve
#                     on these plates draws down 6-11 mg/L.
CURV_MIN_RT     <- 0.5
MIN_O2_DRAWDOWN <- 2.0
...
stable_window <- function(tt, yy) {
  ...
  o2_min <- min(yy[pk:n])
  dd_tot <- o2_pk - o2_min
  if (!is.finite(dd_tot) || dd_tot <= 0) return(NULL)

  # ---- candidate STARTS ------------------------------------------------------
  starts <- integer(0)
  for (om in STAB_START_OFFSETS_MIN) {           # peak-anchored (in minutes)
    s <- which(tt >= tt[pk] + om)
    if (length(s)) starts <- c(starts, s[1])
  }
  for (f in STAB_START_DD_FRACS) {               # drawdown-anchored
    thr <- o2_pk - f * dd_tot
    s <- which(seq_along(tt) >= pk & yy <= thr)
    if (length(s)) starts <- c(starts, s[1])
  }
  starts <- sort(unique(starts))
  starts <- starts[starts <= n - STAB_MIN_PTS]
  ...
  for (s in starts) {
    ...
    # (i) ends by FRACTION OF DRAWDOWN
    r_prov <- NA_real_
    ...
    # (ii) ends by TARGET rt (two-pass: rt depends on r depends on window)
    if (length(STAB_RT_TARGETS) && is.finite(r_prov) && r_prov > 0) {
      for (tgt in STAB_RT_TARGETS) {
        wlen <- tgt / r_prov
        okw <- which(tt[s:n] <= tt[s] + wlen)
        if (!length(okw)) next
        e <- s + max(okw) - 1L
        cc <- try_window(s, e)
  ...
  # drop windows that extrapolate the exponential too far
  if (is.finite(STAB_MAX_RT)) {
    ...
    if (any(keep)) {
      ...
      cd <- cd[1L, , drop = FALSE]      # last resort: shortest extrapolation
    }
  }
  ...
  list(start = best$start[1], end = best$end[1], r = best$r[1], r2 = best$r2[1],
       n_good = nrow(cd), stability_cv = cv, rt = rt, drawdown = drawdown,
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      helpText("Picks the window whose growth rate r is STABLE (barely changes ",
               "when the window is nudged), rather than the window that merely ",
               "looks geometrically tidy. Also flags curves where r is not ",
               "measurable at all."),
      fluidRow(
        column(6, actionButton("stab_one", "Stable-r THIS curve", width = "100%")),
        column(6, actionButton("stab_all", "Stable-r ALL curves", width = "100%"))
      ),
      br(),
      fluidRow(
        column(12, actionButton("stab_temp", "Stable-r THIS TEMPERATURE (all its curves)",
                                width = "100%"))
      ),
      verbatimTextOutput("stab_msg"),
      hr(),
...
server <- function(input, output, session) {
  ...
  stab_flags <- reactiveVal(data.frame(key = character(0), r = numeric(0),
                                       stability_cv = numeric(0), rt = numeric(0),
                                       drawdown = numeric(0),
                                       no_growth = logical(0),
                                       stringsAsFactors = FALSE))

  .stab_verdict <- function(sw) {
    if (is.null(sw)) return("NO reliable window (r not measurable)")
    if (isTRUE(sw$no_growth)) {
      why <- c(if (isTRUE(sw$no_curv)) "no curvature (straight line)" else NULL,
               if (isTRUE(sw$no_signal)) sprintf("no signal (only %.1f mg/L O2 used)", sw$drawdown) else NULL)
      return(paste("ZERO GROWTH:", paste(why, collapse = " + ")))
    }
    if (is.finite(sw$stability_cv) && sw$stability_cv < 0.10) "r is STABLE -> trustworthy"
    else if (is.finite(sw$stability_cv) && sw$stability_cv < 0.25) "r is moderately stable"
    else "r is UNSTABLE -> treat with caution"
  }

  .stab_record <- function(k, sw) {
    f <- stab_flags()
    f <- f[f$key != k, , drop = FALSE]
    if (!is.null(sw)) {
      f <- rbind(f, data.frame(key = k, r = sw$r, stability_cv = sw$stability_cv,
                               rt = sw$rt, drawdown = sw$drawdown,
                               no_growth = sw$no_growth, stringsAsFactors = FALSE))
    } else {
      f <- rbind(f, data.frame(key = k, r = NA_real_, stability_cv = NA_real_,
                               rt = NA_real_, drawdown = NA_real_,
                               no_growth = TRUE, stringsAsFactors = FALSE))
    }
    stab_flags(f)
  }

  stab_txt <- reactiveVal("")
  output$stab_msg <- renderText(stab_txt())
  ...
  observeEvent(input$stab_one, {
    ...
    .stab_record(k, sw)
    if (is.null(sw)) {
      stab_txt("Stable-r: NO reliable window for this curve (r not measurable).")
      showNotification("Stable-r found no reliable window for this curve.", type = "warning")
      return()
    }
    set_val(k, "fit_start", round(sw$start, 1))
    set_val(k, "fit_end",   round(sw$end,   1))
    stab_txt(sprintf(paste0("Stable-r: %.1f - %.1f min | r = %.5f /min (%.4f /h)\n",
                            "R2 = %.3f | %d good windows | stability CV = %.2f | rt = %.2f e-foldings\n",
                            "O2 drawdown = %.1f mg/L\n%s"),
                     sw$start, sw$end, sw$r, sw$r * 60, sw$r2, sw$n_good,
                     sw$stability_cv, sw$rt, sw$drawdown, .stab_verdict(sw)))
  })

  .stab_run <- function(keys, label) {
    w <- wins(); n <- length(keys); nset <- 0L; nbad <- 0L
    withProgress(message = sprintf("Stable-r: %s...", label), value = 0, {
      for (i in seq_along(keys)) {
        ...
        if (!is.null(sw)) {
          ...
          nset <- nset + 1L
          if (isTRUE(sw$no_growth)) nbad <- nbad + 1L
        } else nbad <- nbad + 1L
        incProgress(1 / n)
      }
    })
    ...
    wins(w)
    f <- stab_flags()
    unrel <- f$key[(!is.na(f$no_growth) & f$no_growth) |
                   (is.finite(f$stability_cv) & f$stability_cv >= 0.25)]
    msg <- sprintf(paste0("Stable-r done (%s). Windows set on %d of %d curves.\n",
                          "Flagged UNRELIABLE / zero-growth: %d\n%s"),
                   label, nset, n, length(unrel),
                   if (length(unrel)) paste(" -", paste(unrel, collapse = "\n - ")) else "")
    stab_txt(msg)
    showNotification(sprintf("Stable-r done: %d/%d windows set, %d flagged.",
                             nset, n, length(unrel)), type = "message")
  }

  observeEvent(input$stab_all, .stab_run(curves$key, "all curves"))

  observeEvent(input$stab_temp, {
    Tsel <- curves$T[match(input$curve, curves$key)]
    keys <- curves$key[curves$T == Tsel]
    .stab_run(keys, sprintf("T = %g", Tsel))
  })
