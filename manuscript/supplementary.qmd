...
**Figure S18. Dissolved-oxygen traces and model fits underlying the prothioconazole × SHAM factorial (Fig. 7).** All 132 fitted wells: six plates (rows; temperature × independent culture) × six conditions (columns). Light lines, full raw traces (after removal of the offset caused by briefly taking the plates out at 41 h for an interim export, measured per well in `tables/aox/ptc_sham_steps.csv`); solid lines, the prepared trace (4 h centred moving average) over the fitting interval, which is what was fitted; dashed black lines, the fitted oxygen model O(t) = O₀ + (K/r)(1 − e^{rt}), fitted by bounded nonlinear least squares over that interval (per-well r values, h⁻¹, are printed in each panel and listed with the corresponding unsmoothed fits in `tables/aox/ptc_sham_well_rates.csv`). On the 27 °C plates the vehicle wells approach oxygen depletion by 40 h, so their intervals end there, whereas the drug + SHAM wells are still declining at 64 h. After the plates were moved to a second incubator at about 15 h, the cell-containing wells on the 27 °C plates show a 3 to 4 h sawtooth that is absent from the cell-free blanks on the same plates and therefore reflects periodic re-aeration of the medium rather than a sensor artefact; it is common to every well and is removed by the 4 h moving average applied before fitting.
